...
namespace Sample
{
    ...
    public static class MonitorUpdaterManagerSample
    {
        ...
        private static readonly ILog Log = LogManager.GetLogger(nameof(MonitorUpdaterManagerSample));

        #endregion

        #region Metodos

        public static void UpdateMonitor(string monitorFilesLocation, string installationFolder)
        {
            try
            {
                // ?
                ...
                if (updateError)
                {
                    ...
                    if (!string.IsNullOrEmpty(result))
                    {
                        Log.Info("MonitorUpdater", new Exception(result));
                    }
                    else
                    {
