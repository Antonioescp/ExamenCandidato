...
namespace Sample
{
    ...
    {
        private static readonly ILog Log = LogManager.GetLogger(nameof(FileUpdater));
        ...
        private const string processName = "psample";
        ...
        private static void BackupFile(string backupDir, string targetFolder, string originalFileName, string command)
        {
            ...
            if (File.Exists(originalFileName))
            {
                if (command == DeleteCommandExtension)
                {
                    ...
                    targetRelativeDirectory = Path.GetDirectoryName(GetRelativePath(
                        originalFileName,
                        targetFolder));
                }
        ...
        {
            // Failing to execute a command is assumed to be an invalid update state
            // therefor, the exception is left to be handled by the outer catch block
            string targetFile = file;
            try
            {
                if (createBackup)
                {
                    BackupFile(backupDir, targetRelativeDirectory, file, ExecuteCommandExtension);
                }

                // Windows associates programs to file extensions, if is not .bat nor .cmd then
                // it will not be executed as a cmd script
                if (extension != ".bat" || extension != ".cmd")
                {
                    // Creating temporary .bat file so cmd can execute it properly
                    targetFile = Path.Combine(backupDir, targetRelativeDirectory, $"{fileName}.bat");
                ...
                var processInfo = new ProcessStartInfo
                {
                    ...
                    // "<" could be used instead of "/c" to avoid creating a 
                    // file copy, however "<" causes some cmd statements
                    // to not work properly
                    Arguments = $"/c \"{targetFile}\"",
                ...
                process.OutputDataReceived += (sender, @event) =>
                {
                    ...
                    {
                        Log.Info($"Execution Command \"{fileName}\": {@event.Data}");
                    }
                };
                ...
                process.ErrorDataReceived += (sender, @event) =>
                {
                    if (!string.IsNullOrEmpty(@event.Data))
                    {
                        Log.Error($"Execution Command \"{fileName}\": {@event.Data}");
                    }
                };
        ...
        {
            // File overwrite is assumed to be valid
            // Other exceptions are left to be handled
